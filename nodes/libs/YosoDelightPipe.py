...
class YosoDelightPipeline(StableDiffusionControlNetPipeline):
    ...
    def prepare_latents(
        ...
        该函数旨在对给定的图像进行编码，生成相应的潜变量，并根据需要重复这些潜变量以形成一个集合。
        同时，它也负责生成对应的预测潜变量。

        参数:
        - image: 输入图像张量。
        - latents: 可选的潜变量张量，如果提供，则直接使用，否则将生成一个新的潜变量张量。
        - generator: 可选的随机数生成器，用于生成潜变量。
        - ensemble_size: 集合大小，表示潜变量的重复次数。
        - batch_size: 批处理大小，用于分批处理输入图像。

        返回:
        - image_latent: 编码后的图像潜变量。
        - pred_latent: 预测潜变量。
        """
        ...
        def retrieve_latents(encoder_output):
            ...
            该内部函数用于从编码器的输出中提取潜变量。如果输出包含latent_dist属性，则调用其mode方法；
            如果输出包含latents属性，则直接返回该属性。如果两者都不包含，则抛出异常。
